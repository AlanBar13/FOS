...
export class TableService {
  ...
  async createMultipleTables(amount: number, url: string): Promise<Table[]> {
    const response = await this._apiService.post<Table[]>(
      `/admin/table/multiple/${amount}`,
      { url },
    );
  ...
  async getActiveOrder(tableId: string): Promise<RawOrder> {
    return (await this._apiService.get<RawOrder>(`/table/${tableId}/order`))
      .data;
  }
  ...
  async addOrderItem(
    ...
    return (
      await this._apiService.post(`/table/${tableId}/order/${order}/add`, item)
    ).data;
  }
}

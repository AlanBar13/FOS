...
export class CategoryService {
  ...
  async fetchCategories(): Promise<Category[]> {
    const response =
      await this._apiService.get<Category[]>(`/admin/categories`);
    return response.data;
  }

  async addCategory(item: Category): Promise<Category> {
    const response = await this._apiService.post<Category>(
      `/admin/categories`,
      item,
    );
    return response.data;
  ...
  async updateCategory(id: number, menu: Category): Promise<Category> {
    const response = await this._apiService.patch<Category>(
      `/admin/categories/${id}`,
      menu,
    );
    return response.data;

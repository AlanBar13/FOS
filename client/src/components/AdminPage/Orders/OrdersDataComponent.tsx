...
export default function OrdersDataComponent({
  orders,
  onTableChange,
}: OrdersDataComponentProps) {
  ...
  const visibleRows = useMemo(() => {
    return orders
      .sort(createCompareFn(orderBy, order))
            ...
            <TableRow>
              {properties.map((property) => (
                <TableCell
                  key={property.name}
                  sortDirection={
                    orderBy === (property.name as keyof RawOrder)
                      ? order
                      : false
                  }
                >
                  {property.noSort ? (
                    ...
                    <TableSortLabel
                      ...
                      onClick={() =>
                        handleSort(property.name as keyof RawOrder)
                      }
                    >
                      {property.displayName.toLocaleUpperCase()}
                    </TableSortLabel>
          ...
          <TableBody>
            {visibleRows.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>
                  {formatPriceFixed(item.subtotal ? item.subtotal : 0)}
                </TableCell>
                <TableCell>
                  {formatPriceFixed(item.taxTotal ? item.taxTotal : 0)}
                </TableCell>
                <TableCell>{formatPriceFixed(item.total)}</TableCell>
                <TableCell>
                  <Chip
                    sx={{ background: getOrderStatusColor(item.status).color }}
                    label={`${getOrderStatusColor(item.status).name}`}
                  />
                </TableCell>

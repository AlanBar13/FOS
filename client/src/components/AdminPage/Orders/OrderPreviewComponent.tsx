...
export default function OrderPreviewComponent() {
  ...
  const handleChangeQty = (
    ...
    if (event.target.value === "") {
      ...
      if (qty > 0) setQty(qty);
      else showAlert("Cantidad tiene que ser mayor a 0", "warning");
    }
  };
  ...
  const createItem = async () => {
    try {
      ...
      socket.emit(SocketEvents.clientSendOrder, data.id, data.tableId, [
        dashboardItem,
      ]);
      setOpen(false);
      ...
      showAlert(
        `Error al crear nuevo producto a la order ${data?.id}`,
        "error",
      );
      setModalLoading(false);
    }
  ...
  return (
    <>
      ...
      {isLoading ? (
          ...
          >
            <div className="order-preview_data">
              ...
              <Typography variant="h6">
                Status: {getOrderStatusSring(data.status)}
              </Typography>
            </div>
            <Divider orientation="vertical" flexItem />
            <div className="order-preview_data">
              <Typography variant="h6">
                Subtotal: {formatPriceFixed(data.subtotal)}
              </Typography>
              <Typography variant="h6">
                IVA: {formatPriceFixed(data.taxTotal)}
              </Typography>
            </div>
            <Divider orientation="vertical" flexItem />
            <div className="order-preview_data">
              <Typography variant="h6">
                Total: {formatPriceFixed(data.total)}
              </Typography>
              <Typography variant="h6">
                Propina: {formatPriceFixed(data.tips)}
              </Typography>
            </div>
            <Divider orientation="vertical" flexItem />
            <div className="order-preview_data">
              <Typography variant="h6">
                Creado en: {formatStringDate(data.createdAt)}
              </Typography>
              <Typography variant="h6">
                Actualizado en: {new Date(data.updatedAt).toLocaleString()}
              </Typography>
            </div>
          </Paper>
        ...
        <div>
          <Typography>Orden No disponible</Typography>
        </div>
      )}

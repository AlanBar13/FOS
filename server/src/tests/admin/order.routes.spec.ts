  ...
  describe("POST /v1/admin/order/:id/status/change/:status", () => {
    ...
    before(async () => {
      response = await request(server).post(
        `/v1/admin/order/${orderId}/status/change/${status}`,
      );
    });
    ...
    after(async () => {
      await request(server).post(
        `/v1/admin/order/${orderId}/status/change/paid`,
      );
    });
  });
  ...
  describe("POST /v1/admin/order/:id/status/ready", () => {
    ...
    it("should return status 400", async () => {
      const response = await request(server).post(
        `/v1/admin/order/${orderId}/status/ready`,
      );
    ...
    it("should return status 200", async () => {
      let response = await request(server).post(
        `/v1/admin/order/${orderId}/status/change/inKitchen`,
      );
      response = await request(server).post(
        `/v1/admin/order/${orderId}/status/ready`,
      );

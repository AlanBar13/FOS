...
export const registerOrderItem = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (
      ...
      order.status === OrderStatus.userClosed
    ) {
      res.status(400);
...
export const getItemsByOrder = asyncHandler(
  async (req: Request, res: Response) => {
    const orderId = Number(req.params.orderId);
...
export const deleteItemByOrderId = asyncHandler(
  async (req: Request, res: Response) => {
    const orderId = Number(req.params.orderId);
...
export const updateItemCommentsByOrderId = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      const orderId = Number(req.params.orderId);
      const itemId = Number(req.params.itemId);
      ...
      res.json({
        message: `Item ${itemId} comments were updated from order ${orderId}`,
      });
    } catch (error) {
